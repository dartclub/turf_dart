...
Future<void> generateVisualization() async {
  try {
    ...
    for (final filename in files) {
      ...
      if (outputJson['type'] == 'FeatureCollection' &&
          outputJson['features'] is List) {
          ...
          if (feature['properties'] != null &&
            ...
            if (filename == 'point.geojson') {
              feature['properties']['description'] =
                  'Point on a Point: Returns the original point unchanged';
              feature['properties']['marker-symbol'] = 'star';
            } else if (filename == 'linestring.geojson') {
              feature['properties']['description'] =
                  'Point on a LineString: Returns the midpoint of the first segment';
              feature['properties']['marker-symbol'] = 'circle';
            } else if (filename.contains('polygon') &&
                !filename.contains('multi')) {
              feature['properties']['description'] =
                  'Point on a Polygon: Returns a point inside the polygon (prefers centroid)';
              feature['properties']['marker-symbol'] = 'triangle';
            } else if (filename == 'multipolygon.geojson') {
              feature['properties']['description'] =
                  'Point on a MultiPolygon: Returns a point from the first polygon';
              feature['properties']['marker-symbol'] = 'square';
            } else if (filename == 'featurecollection.geojson') {
              feature['properties']['description'] =
                  'Point on a FeatureCollection: Returns a point on the largest feature';
    ...
    final visualizationFile =
        File('test/examples/point_on_feature/visualization.geojson');
    await visualizationFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(featureCollection.toJson()));
...
void setFeatureStyle(Feature feature, String color, int width, double opacity) {
  ...
  if (feature.geometry is Polygon || feature.geometry is MultiPolygon) {
    ...
    feature.properties!['fill-opacity'] = opacity;
      ...
      feature.geometry is MultiLineString) {
    feature.properties!['stroke'] = color;
    feature.properties!['stroke-width'] = width;
    feature.properties!['stroke-opacity'] = 1;
