...
void main() {
  group('flatten', () {
    test(
        'Point geometry - should return a FeatureCollection with a single Point feature',
        () {
      ...
      expect((result.features[0].geometry as Point).coordinates,
          equals(Position(1, 2)));
    });

    test(
        'MultiPoint geometry - should return a FeatureCollection with multiple Point features',
        () {
      var multiPoint =
          MultiPoint(coordinates: [Position(1, 2), Position(4, 5)]);
      ...
      expect((result.features[1].geometry as Point).coordinates,
          equals(Position(4, 5)));
    });

    test(
        'LineString geometry - should return a FeatureCollection with a single LineString feature',
        () {
      var lineString =
          LineString(coordinates: [Position(1, 2), Position(4, 5)]);
    ...
    test(
        'MultiLineString geometry - should return a FeatureCollection with multiple LineString features',
        () {
    ...
    test(
        'Polygon geometry - should return a FeatureCollection with a single Polygon feature',
        () {
    ...
    test(
        'MultiPolygon geometry - should return a FeatureCollection with multiple Polygon features',
        () {
    ...
    test('Feature with Point geometry - should preserve properties', () {
      ...
      expect(result.features[0].properties,
          equals({'name': 'Test Point', 'value': 42}));
    ...
    test(
        'Feature with MultiPoint geometry - should preserve properties in all output features',
        () {
      var feature = Feature<MultiPoint>(
          geometry: MultiPoint(coordinates: [Position(1, 2), Position(4, 5)]),
      ...
      for (var feat in result.features) {
        expect(
            feat.properties, equals({'name': 'Test MultiPoint', 'value': 42}));
      }
    });

    test(
        'Altitude preservation - should retain altitude (z) values in coordinates',
        () {
    ...
    test('Comprehensive altitude preservation test', () {
      ...
      var multiLineString = MultiLineString(coordinates: [
        [Position(1, 2, 10), Position(3, 4, 20), Position(5, 6, 30)],
        [Position(7, 8, 40), Position(9, 10, 50)]
      ]);
    ...
    test(
        'FeatureCollection with mixed geometries - should flatten all Multi* geometries',
        () {
      var featureCollection = FeatureCollection<GeometryObject>(features: [
        Feature<Point>(geometry: Point(coordinates: Position(1, 2))),
        Feature<MultiPoint>(
            geometry:
                MultiPoint(coordinates: [Position(4, 5), Position(7, 8)])),
        Feature<LineString>(
            geometry:
                LineString(coordinates: [Position(10, 11), Position(13, 14)])),
