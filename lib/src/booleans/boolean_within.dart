...
isMultiPointInMultiPoint(MultiPoint multiPoint1, MultiPoint multiPoint2) {
  for (var i = 0; i < multiPoint1.coordinates.length; i++) {
    ...
    for (var i2 = 0; i2 < multiPoint2.coordinates.length; i2++) {
      if (compareCoords(
          multiPoint1.coordinates[i], multiPoint2.coordinates[i2])) {
        anyMatch = true;
      }
    }
    if (!anyMatch) {
...
function isLineOnLine(LineString lineString1, LineString lineString2) {
  for (var i = 0; i < lineString1.coordinates.length; i++) {
    if (!booleanPointOnLine(lineString1.coordinates[i], lineString2)) {
      return false;
    }
 ...
 * @returns {boolean} true/false if coord pairs match
 */
function compareCoords(Position pair1, Position pair2) {
  return pair1[0] == pair2[0] && pair1[1] == pair2[1];
}
 ...
 * @returns {Position} midpoint of pair1 and pair2
 */
function getMidpoint(Position pair1, Position pair2) {
  return [(pair1[0] + pair2[0]) / 2, (pair1[1] + pair2[1]) / 2];
}
