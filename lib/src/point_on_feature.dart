  ...
  if (featureInput is Feature) {
    ...
    if (geometry is Point) {
      ...
      return Feature<Point>(
          geometry: geometry, properties: featureInput.properties);
      ...
      final pointPos = Position(
          centroid.coordinates[0] ?? 0.0, centroid.coordinates[1] ?? 0.0);

      // Use point-in-polygon from turf_pip package directly
      final pipResult = pointInPolygon(Point(coordinates: pointPos), geometry);
      if (pipResult == PointInPolygonResult.isInside ||
          pipResult == PointInPolygonResult.isOnEdge) {
        ...
        for (final pos in outerRing) {
          ...
          final candidatePos = Position(
              candidate.coordinates[0] ?? 0.0, candidate.coordinates[1] ?? 0.0);
          final candidatePipResult =
              pointInPolygon(Point(coordinates: candidatePos), geometry);
          if (candidatePipResult == PointInPolygonResult.isInside ||
              candidatePipResult == PointInPolygonResult.isOnEdge) {
            return Feature<Point>(
                geometry: candidate, properties: featureInput.properties);
          }
        }
      ...
      if (geometry.coordinates.isNotEmpty &&
        ...
        return pointOnFeature(
            Feature(geometry: firstPoly, properties: featureInput.properties));
      }
...
Feature<Point> _midpointOnLine(
    LineString line, Map<String, dynamic>? properties) {
  final coords = line.coordinates;
  if (coords.isEmpty) {
    // Fallback for empty LineString - should not happen with valid GeoJSON
    return Feature<Point>(
        geometry: Point(coordinates: Position(0, 0)), properties: properties);
  }

  if (coords.length == 1) {
    // Only one point in the LineString
    return Feature<Point>(
        geometry: Point(coordinates: coords.first), properties: properties);
  }
  ...
  return Feature<Point>(geometry: midpoint, properties: properties);
}
...
double _calculateFeatureSize(Feature feature) {
  ...
  if (geometry is Point) {
    ...
    final num calculatedLength = turf_length.length(
        Feature<LineString>(geometry: geometry), Unit.kilometers);
