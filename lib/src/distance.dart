...
double distanceRaw(Position from, Position to, [Unit unit = Unit.kilometers]) {
  ...
  double a = pow(sin(dLat / 2), 2).toDouble() +
      pow(sin(dLon / 2), 2).toDouble() * cos(lat1) * cos(lat2);
...
/// ```
double distance(Point from, Point to, [Unit unit = Unit.kilometers]) =>
